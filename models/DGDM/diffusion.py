...
class GaussianDiffusion(nn.Module):
    ...
    def p_sample(self, x, t: int, cond=None, ctx=None, idx=None, ):
        ...
        model_mean, _, model_log_variance, x_start = self.p_mean_variance(x=x, t=batched_times, cond=cond, ctx=ctx,
                                                                          idx=idx, clip_denoised=True)
    ...
    def ddim_sample(self, shape, cond=None, ctx=None, idx=None, return_all_timesteps=False):
        batch, total_timesteps, sampling_timesteps, eta, objective = shape[
            0], self.num_timesteps, self.sampling_timesteps, self.ddim_sampling_eta, self.objective
        ...
        for time, time_next in tqdm(time_pairs, desc='sampling loop time step', disable=True):
            ...
            pred_noise, x_start, *_ = self.model_predictions(frames_pred, time_cond, cond=cond, ctx=ctx, idx=idx,
    ...
    def predict(self, frames_in, compute_loss=False, **kwargs):
        ...
        if compute_loss:
            raise NotImplementedError(
                "We are sorry that we do not support training process for now because of business limitation ")
        return pred, None
