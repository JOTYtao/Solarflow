...
class Determinisitic(nn.Module):
    def __init__(self, shape_in=(8, 1), hid_S=64, hid_T=512, N_S=6, N_T=8, out_frames=8):
    ...
    def forward(self, x_raw):
        x_raw = rearrange(x_raw, 'b c t h w -> b t c h w')
        ...
        Y = rearrange(Y, 'b (t c) h w -> b c t h w', t=self.out_frames)
        return Y, feature

if __name__ == "__main__":
    import numpy as np
    model = Determinisitic()
    inputs = torch.randn(16, 1, 8, 128, 128)
