...
def extras(config: DictConfig) -> None:
    ...
    if "trainer" not in config:
        log.warning("No trainer configuration found in config. Adding empty trainer config.")
        config.trainer = OmegaConf.create({})
    trainer_config = config.trainer
    acc_grad_batches = trainer_config.get("accumulate_grad_batches", None)
    if acc_grad_batches:
        limit_train_batches = trainer_config.get("limit_train_batches", 1.0)
        trainer_config.limit_train_batches = limit_train_batches * acc_grad_batches
