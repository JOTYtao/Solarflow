...
def inverse_rescale_data(data: torch.Tensor, min_val: float = 0.0, max_val: float = 1.2) -> torch.Tensor:
    return ((data + 1) / 2) * (max_val - min_val) + min_val
def metric(pred, true, metrics=['mae', 'mse', 'ssim', 'lpips'], min_val=0.0, max_val=1.2,
    ...
    """
    pred = inverse_rescale_data(pred, min_val, max_val)
    true = inverse_rescale_data(true, min_val, max_val)
    eval_res = {}
    eval_log = ""
