...
class WandbLogger(Callback):
    ...
    def _log_images(
        ...
        n = min(self.num_images, input_images.shape[0])
        inputs = input_images[:n].permute(0, 1, 3, 4, 2).cpu().numpy()
        preds = pred_images[:n].permute(0, 1, 3, 4, 2).cpu().numpy()
        trues = true_images[:n].permute(0, 1, 3, 4, 2).cpu().numpy()
        for idx in range(n):
            images = []
            images.append(wandb.Image(
                inputs[idx, -1],
                caption=f"Input (t={input_images.shape[1] - 1})"
            ))
            images.append(wandb.Image(
                preds[idx, 0],
                caption="Prediction (t=0)"
            ))
            images.append(wandb.Image(
                preds[idx, -1],
                caption=f"Prediction (t={pred_images.shape[1] - 1})"
            ))
            images.append(wandb.Image(
                trues[idx, 0],
                caption="Ground Truth (t=0)"
            ))
            images.append(wandb.Image(
                trues[idx, -1],
                caption=f"Ground Truth (t={true_images.shape[1] - 1})"
            ))
            wandb.log({
                f"{mode}/sample_{idx}/sequence": images,
            }, step=step)
    def on_train_batch_end(
        ...
        for i, param_group in enumerate(opt.param_groups):
            trainer.logger.experiment.log({
                f"train/lr_group_{i}": param_group['lr']
            }, step=trainer.global_step)
        if self.log_train_images:
            input_data = batch["his_cal"].permute(0, 2, 1, 3, 4).contiguous()
            target_data = batch["target"].permute(0, 2, 1, 3, 4).contiguous()
            pred_data = pl_module(input_data, target_data)
            self._log_images(
                trainer,
                pl_module,
                input_data,
    ...
    def on_validation_batch_end(
        ...
        if not self.log_val_images or batch_idx % self.log_freq != 0:
            return
